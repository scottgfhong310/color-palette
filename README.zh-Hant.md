...
- **拖拉／點選上傳** 圖片（`.png` `.jpg` `.webp` `.avif` `.gif` `.bmp`），上傳後自動分析。
...
- **明細五視圖**：點 alias 可看原圖與各色的 hex／佔比，並在**色族／主色／全收／分布／重點色**五種顏色類型間切換（面積代表色、ΔE≈5 感知分布、彩度加權顯著性）——各類型的意義與選用見 [COLOR-TYPES.md](COLOR-TYPES.md)。
- **色彩肖像**：把五種色型跨層比較，即時生成一句描述（暖冷、面積主導、明度/彩度、焦點色、以及「看似中性實則某色主導」這類跨構面張力），焦點色以在地化的 Faber-Castell 名命名、並相對整個圖庫給觀點——想法到設計見 [COLOR-PORTRAIT.md](COLOR-PORTRAIT.md)。
