...
- **燈箱點原圖＝釘住 ＋ 複製 css hex**（同一句話的第二個場合）：滑過是預覽、**點下去就是「我要這個色碼」**，
  不必再去點一次取色頭。取色頭仍可再點一次重複製（比對完想再拿一次時）；✕ 取消釘選不複製。
  ⚠️ **只在 `pinAt()` 複製、不在 `eyedrop()`**——滑過取色是連續事件，在那裡複製等於滑鼠一移過去
  就無聲改寫剪貼簿。**「點擊」是使用者手勢、「滑過」不是**，而 `clipboard.writeText` 也只在前者被允許。
