...
✅ **已實作（MVP）**：`ColorPortraitLib.tags(desc)` 把 Description 攤成一組正規化標籤（`facet:value`，如
`temp:cool`／`archetype:earthy`／`harmony:complementary`／`key:mid`）——這就是**可查詢的色彩身分**。
gallery 的 **篩選列**（側鍵 `filter_list`）依現有標籤動態生成 chip（溫度／原型／和諧／明度），
點擊即時過濾（**同 facet 內 OR、跨 facet AND**）。**標籤在分析時由 live 分布算準確值，隨 alias 一起落地
`.registry.json`**（後端 `validateTags` 驗證後存 `alias.tags`）——持久、可跨載入檢索，`/files` 直接帶出。
舊資料尚未落地 tags 時，後備由 alias 色**近似**算（zero-後端負擔），重新分析即升級為準確值。
...
- **選配 LLM 潤稿** ✅ **已實作**：把決定論那一句（＋精簡事實護欄）丟給模型改寫得更漂亮——
  **這裡才讓主觀與文采進場**，且嚴格 opt-in、純核心仍決定論、零相依：
  ...
  - **零 npm 相依 · 支援兩家**：後端以 Node 內建 `fetch` 直呼**所選 provider**（不引 SDK，維持「薄後端」canon）——
    `.env` 的 `LLM_PROVIDER` 選 `anthropic`（預設，Messages API）或 `openai`（Chat Completions），金鑰走 `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`、
    模型走 `ANTHROPIC_MODEL`/`OPENAI_MODEL`（預設 `claude-opus-4-8`/`gpt-4o-mini`）。`callLLM()` 內兩家共用同一份 prompt 與護欄，只換傳輸層。
...
（items＝`{colors, mtime, tags}`）→ `collectionPhrase()` 出一行摘要，顯示在 gallery 頂端（`#collection-portrait`）：

> 「83 張 · 整體偏暖 · 單色為主 · **近期偏暖**」

- **關係**：整體冷暖、最常見的配色原型/和諧（≥30% 才說「多為」）；**尊重篩選**——描述的是目前**可見的子集**
  （篩「互補」→「9 張 · … · 互補為主」），所以也能拿來描述任意過濾出的集合。
- **時間**：依 `mtime` 把該批分較舊/較新兩半，比暖冷與彩度差 → **趨勢**（近期偏暖/冷/鮮/濁）。
- 原型/和諧用每張圖的**落地 tags**（與篩選同源，一致）；暖冷/彩度趨勢用 `metrics()`。
- 未做：相對「某個外部畫派/攝影師」的比較（需外部參考集）——目前只相對「本圖庫/本子集」。
- **怎麼理解（尤其「時間」怎麼呈現）＋用哪些資料，見 [COLLECTION.md](COLLECTION.md)。**
