...
- **明細萃取視圖分頁**（對齊 `thangka-trace` 的用色清單面板）：色族（median）／主色（frequency）／全收（不濾近白黑，含紙底/線稿）／**分布**／**重點色**，
  ...
  工具列另有**複製全部色碼**、**存 `.md`**（單構面：左圖右表）與**完整報告 `summarize`**（`buildReportMd`：五構面全放、每段 cap 16 列、總覽比例色帶＋分段、分頁點 `break-inside:avoid` 設在段邊界）——皆 SVG 色塊必印、交 `markdown-library` 以 `?mymd` 絕對路徑開啟。落地 alias 仍是 hue-sorted 指紋，分頁為即時檢視、不改 registry。
