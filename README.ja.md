...
- 画像の **ドラッグ&ドロップ／クリックアップロード**（`.png` `.jpg` `.webp` `.avif` `.gif` `.bmp`）、アップロード時に自動解析。
...
- **カラーポートレート**：5 つの色タイプを横断的に比較し、一文をその場で生成（暖寒・面積主導・キー/彩度・焦点色、および「一見ニュートラルだが実は◯が主導」のような視点間の緊張）。焦点色はローカライズした Faber-Castell 名で示し、ライブラリ全体に対する視点も添える——構想から設計は [COLOR-PORTRAIT.md](COLOR-PORTRAIT.md)。
