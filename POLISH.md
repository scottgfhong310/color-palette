...
| **ollama** · `qwen2.5:7b`（本機） | 整體暖調、採用互補色彩，呈現溫厚大地風貌，焦點為一抹鮮豔的小印度紅（FC192）。 |
| **ollama** · `qwen2.5:14b`（區網另一台） | 一抹温暖的印度紅 ≈FC192 在大地色調中獨具焦點，與整體暖色系形成鮮明互補對比。 |

> **本機/自架的遵守度隨模型大小而變**（`ollama` 走同一份 prompt／護欄）：
> - `qwen2.5:7b`——**較弱**：把 `≈` 去掉、改用括號寫成「（FC192）」（`normalizeFC` 只正規化既有的 `≈FC` token，不替模型補回 `≈`）；多跑還偶爾把英文事實標籤（Warm／Earthy／India Red）漏進句子、或混簡體。
> - `qwen2.5:14b`——**明顯較好**：**守住 `≈FC192`**、不漏標籤；僅偶爾一個繁簡字（此例「温」應為「溫」）。
>
> 想更貼近雲端遵守度就用更大的本機模型（14b＞7b，或 32b）或雲端 provider。`ollama` 也可指向**另一台主機**：`OLLAMA_BASE_URL=http://192.168.0.x:11434`（見 §1.2）；那台關機/不可達則潤稿優雅失敗、不影響其他功能。

**看出幾件事：**
- **雲端都忠實；本機/自架看模型大小**——雲端三句都只講顏色、一句、`≈FC192` 原樣保留（空格由 `normalizeFC` 統一）。共用的 system prompt＋事實護欄在管，但**遵守度隨模型強弱而變**：`qwen2.5:7b` 偶爾去掉 `≈`／漏英文標籤／混簡體，`qwen2.5:14b` 就守得住 `≈FC192`（見上註）——護欄是「盡力」不是「保證」。
...
潤稿是模型寫的，**焦點色的 Faber-Castell 參照** `≈FC192` 各家寫法不一——尤其本機/小模型：漏掉 `≈` 前後的半形空格、把 `≈`（近似符號）翻成中文「約」「大約」或寫成波浪號 `~`／`～`。後端在**出字後**用純函式 `normalizeFC()` 統一（**決定論核心不受影響、三家 provider 都套用**）：

1. **近似標記 → `≈`**：`約`／`大約`／`~`／`～` 緊接 FC碼 → `≈`。
2. **收攏內部空白**：`≈ FC 192` → `≈FC192`（順帶把 `fc` 正規化成大寫 `FC`）。
3. **前後補空格**：`≈FC碼` 前後各恰一個半形空格——**除非**緊鄰 空白／括號（全形半形）／標點／引號／行首尾。

| 模型寫出 | 正規化後 |
|---|---|
| `鈷藍≈FC143。` | `鈷藍 ≈FC143。` |
| `印度紅 ≈FC192格外` | `印度紅 ≈FC192 格外` |
| `（約 FC192）` | `（≈FC192）` |
| `鈷藍~FC143` | `鈷藍 ≈FC143` |

**兩條界線**（保持誠實、不越權）：
- **只碰 `≈FC` token**——句子其餘一字不動。
- **不憑空補 `≈`**——若模型完全沒寫近似標記（如小模型直接寫成 `（FC192）`），就**維持原樣**：寧可如實呈現「模型沒標近似」，也不替它捏一個 `≈`（見 §2.6 的 7b caveat）。
